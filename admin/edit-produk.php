...
$hasil = mysqli_query($conn, "SELECT * FROM produk WHERE idproduk=$id");
while($data = mysqli_fetch_array($hasil))
{
  $vnamaproduk = $data['namaproduk'];
  $vkategori = $data['idkategori'];
  $vbrand = $data['idbrand'];
  $vdeskripsi = $data['deskripsi'];
  $vhargabefore = $data['hargabefore'];
  $vhargaafter = $data['hargaafter'];
}
...
if (isset($_POST["update"])) {
  ...
  // Cek Duplikasi 
    $result = mysqli_query($conn, "UPDATE produk SET namaproduk='$namaproduk', idkategori='$kategori', idbrand='$brand', deksripsi='$deskripsi', hargabefore='$hargabefore', hargaafter='$hargaafter' WHERE idproduk=$id");
    if($result) {
      $berhasil = true;
    } else {
      $error = true;
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3 d-flex justify-content-between align-items-center">
                            ...
                            <button data-toggle="modal" data-target="#myModal" class="btn btn-sm btn-info col-md-2">Tambah Produk</button>
                        </div>
                        <div class="card-body">
                            <a href="produk.php">
                        ...
                        <?php 
                            ...
                            <form action="" method="post">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="namaproduk" name="namaproduk" value="<?= $vnamaproduk ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="norek" class="form-label">Pilih Kategori</label>
                                    <select name="idkategori" class="form-control">
									<option selected><?= $vkategori ?></option>
                                    <?php
                                ...
                                <div class="mb-3">
                                    <label for="an" class="form-label">Nama Brand</label>
                                    <select name="idbrand" class="form-control">
									<option selected><?= $vbrand ?></option>
                                    <?php
									$det=mysqli_query($conn,"SELECT * from brand order by namabrand ASC")or die(mysqli_error($conn));
									while($d=mysqli_fetch_array($det)){
									?>
									<option value="<?= $d['idbrand'] ?>"><?= $d['namabrand'] ?></option>
									<?php } ?>
									</select>
                                </div>
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    <label for="namaproduk" class="form-label">Harga Sebelum Diskon</label>
                                    <input name="hargabefore" type="number" class="form-control" value="<?= $vhargabefore ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="namaproduk" class="form-label">Harga Setelah Diskon</label>
                                    <input name="hargaafter" type="number" class="form-control" value="<?= $vhargaafter ?>">
                                </div>
                                <div class="mb-3">
